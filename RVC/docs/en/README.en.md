...
The easiest way to get started is using Docker! No Python setup required.
...
docker run --name rvc-job rvc-inference:latest python tools/infer_cli.py \
...
# Copy output to host
docker cp rvc-job:/app/output/result.wav ./output/
docker rm rvc-job
...
**For Windows users, use the helper script:**
```powershell
.\rvc-docker.ps1 build
.\rvc-docker.ps1 infer --input_path "assets/TestAudios/Brandon saying.mp3" --model_name "Brandon/Brandon.pth" --opt_path "output/result.wav"
.\rvc-docker.ps1 web
```

---

## Preparing the environment (Manual Installation)
